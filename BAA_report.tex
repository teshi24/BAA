	...
	\chapter{State of Research}
		...
		\section{PASSION for Dermatology}
			...
			\subsection{PASSION Data Analysis Scripts}
				With the Dataset, the PASSION research team provides a \gls{JupyterNotebook} with code examples and data analysis scripts. The scripts are listed in \autoref{tab:PASSION_scripts} with an indicator, how relevant the scripts are for this thesis. Relevant are the scripts which are related to demographic distributions of the chosen labels since they can indicate whether the data distribution might be a issue which leads to biases. \todo{somehow write it so that it is clear that it could LEAD to biases while it not necesserily MUST lead to biases.}. Somewhat relevant are the scripts which lay foundations for further data analysis scripts. Not relevant for this thesis are all other scripts.
				...
				\begin{table}[H]
					...
					\begin{tabularx}{\textwidth}{>{\hsize=.25\hsize\raggedright}X>{\hsize=.41\hsize}X>{\hsize=.34\hsize}X}
						...
						\textbf{Medium} - Basis for other analysis's \\
						\hline
						Extracting and Comparing Subject IDs &
						Checks the dataset complecity and accurracy in regards of linking records and images &
						\textbf{Low} - Not providing more insight in regards of demographic distribution \\
						\hline
						Regrouping Malawi and Tanzania to EAS &
						data aggregation due to dataset size and geographical proximity &
						\textbf{Low} - Only relevant to understand the dataset and for interpreting the results of the following scripts correctly  \\
						\hline
						Conditions by Country &
						Relationship between clinical conditions and country &
						\textbf{Low} - Currenty not relevant since the \textit{country} label is out of the scope of this thesis \\
						\hline
						Body Localizations by Conditions &
						Shows correlation between the condition and primarily affected body parts; does not use all affected body parts listed in the data &
						\textbf{Low} - Not relevant for analyzing demographic distributions. \\
						\hline
						Impetigo Cases            &
						Counts total number of impetigo cases as well as proportion to all cases &
						\textbf{Low} - Not relevant for analyzing demographic distribution, but potentialy relevant for dermatology distribution  \todo{check this hypothesis: research relevance between impedigo and demographic bias} \\
						\hline
						Distribution of \glspl{FST} &
						Counts and visualizes the skin type distribution  &
						\textbf{High} - Analysis of a demographic distribution\\
						\bottomrule
					\end{tabularx}
